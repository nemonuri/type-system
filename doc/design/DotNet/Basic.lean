...
theorem sizeOf_gt_zero {rc: Nat} (tst: TypeStack rc) : sizeOf tst > 0 := by
  unfold SizeOf.sizeOf
  unfold TypeStack._sizeOf_inst
  unfold TypeStack._sizeOf_1
  match tst with
  | .alloc td => simp; omega
  | .push _ _ _ => simp; omega
